...
async def send_task_notification(guild, assignee, instructor, task_name, due_date, original_message_id):
    ...
    # 個人チャンネルが存在しない場合はDMのみ使用する
    if not channel:
        logger.info(f"Personal channel {channel_name} not found for {assignee.display_name}")
        try:
            channel = await assignee.create_dm()
            await channel.send("⚠️ 個人チャンネルが見つかりません。管理者に `!個人チャンネル作成` コマンドの実行を依頼してください。")
        except:
            logger.error(f"Failed to send DM to {assignee.display_name}")
            # DMの送信に失敗した場合、タスク通知の送信を試みる
            channel = None
...
async def create_channels_command(ctx):
    ...
    if not management_channel:
        try:
            ...
            # 作成成功の通知とガイダンス
            embed = discord.Embed(
                title="✅ タスク管理チャンネルを作成しました",
                description="個人タスクチャンネルは必要に応じて個別に作成してください。",
                color=discord.Color.green()
            )
            embed.add_field(
                name="個人チャンネルの作成方法",
                value="`!個人チャンネル作成 @ユーザー` コマンドを使用してください。",
                inline=False
            )
            await ctx.send(embed=embed)
            
            # 管理チャンネルにも案内を送信
            await management_channel.send("✅ タスク管理チャンネルが作成されました。個人タスクチャンネルは `!個人チャンネル作成 @ユーザー` で必要な分だけ作成してください。")
            
        except discord.Forbidden:
            await ctx.send("❌ チャンネル作成権限がありません。")
        except Exception as e:
            logger.error(f"Error creating management channel: {e}")
            await ctx.send("❌ チャンネル作成中にエラーが発生しました。")
    else:
        await ctx.send("ℹ️ タスク管理チャンネルは既に存在します。個人チャンネルは `!個人チャンネル作成 @ユーザー` で作成してください。")
...
async def check_reminders():
    ...
    upcoming_tasks = DatabaseManager.execute_query(
        """
        SELECT id, guild_id, instructor_id, assignee_id, task_name, due_date 
        FROM tasks 
    ...
    for task in upcoming_tasks:
        ...
        if not guild:
            logger.warning(f"Guild {guild_id} not found for task {task_id}")
            DatabaseManager.mark_reminder_sent(task_id)
            continue
        
        assignee = guild.get_member(assignee_id)
        if not assignee:
            logger.warning(f"Assignee {assignee_id} not found in guild {guild_id}")
            DatabaseManager.mark_reminder_sent(task_id)
            continue
        ...
        try:
            ...
            embed = discord.Embed(
                title="⏰ タスクリマインダー",
                description=f"『{task_name}』の期限まであと1時間です",
                color=discord.Color.orange(),
                timestamp=datetime.datetime.now()
            )
            embed.add_field(name="期限", value=due_date.strftime("%Y/%m/%d %H:%M"), inline=True)
            embed.add_field(name="タスクID", value=f"#{task_id}", inline=True)
            
            success = False
            
            # 1. 個人チャンネルがある場合、そこに送信
            channel_name = f"to-{assignee.display_name}"
            channel = discord.utils.get(guild.channels, name=channel_name)
            ...
            if channel and isinstance(channel, discord.TextChannel):
                try:
                    ...
                    success = True
                    logger.info(f"Reminder sent to channel {channel_name} for task {task_id}")
                except Exception as e:
                    logger.error(f"Failed to send reminder to channel {channel_name}: {e}")
            
            # 2. 必ずDMにも送信
            try:
                await assignee.send(embed=embed)
                success = True
                logger.info(f"Reminder sent to DM for task {task_id}")
            except discord.Forbidden:
                logger.warning(f"Could not send reminder DM to {assignee.display_name}: DMs forbidden")
            except Exception as e:
                logger.error(f"Failed to send reminder DM for task {task_id}: {e}")
            
            # 送信試行完了後、成否に関わらずフラグを設定（重複送信を防ぐため）
            DatabaseManager.mark_reminder_sent(task_id)
            
            if not success:
                logger.warning(f"Failed to send any reminder for task {task_id}")
                
        except Exception as e:
            logger.error(f"Error in reminder process for task {task_id}: {e}")
            # エラー時もフラグを設定して重複を防ぐ
            DatabaseManager.mark_reminder_sent(task_id)
