...
async def send_task_notification(guild, assignee, instructor, task_name, due_date, original_message_id):
    ...
    if not channel:
        # チャンネルが存在しない場合の処理
        logger.warning(f"Channel {channel_name} not found for user {assignee.name}")
        ...
        try:
            ...
            logger.warning(f"Cannot send DM to instructor {instructor.name} (DMs forbidden)")
        except Exception as e:
            logger.error(f"Failed to send DM to instructor {instructor.name}: {e}")
        ...
        try:
            ...
            logger.warning(f"Cannot send DM to assignee {assignee.name} (DMs forbidden)")
            return
        except Exception as e:
            logger.error(f"Failed to send DM to assignee {assignee.name}: {e}")
            return
...
async def create_channels_command(ctx):
    ...
    for member in guild.members:
        ...
        # 一貫性のある関数を使用してチャンネル名を取得
        channel_name = get_user_channel_name(member)
        ...
        try:
            ...
            if channel:
                try:
                    ...
                    logger.warning(f"Cannot send reminder to channel {channel_name} (no permission)")
                except Exception as e:
                    logger.error(f"Failed to send reminder to channel {channel_name}: {e}")
            
            # チャンネルがないか送信に失敗した場合はDMで試行
            if not sent_successfully:
                try:
                    await assignee.send(embed=embed)
                    sent_successfully = True
                    logger.info(f"Reminder sent via DM to {assignee.name} for task {task_id}")
                except discord.Forbidden:
                    logger.warning(f"Cannot send DM to {assignee.name} (DMs forbidden)")
                except Exception as e:
                    logger.error(f"Failed to send DM to {assignee.name}: {e}")
            
            # 送信成功・失敗に関わらずフラグを設定（重複送信防止）
            DatabaseManager.mark_reminder_sent(task_id)
            
            if not sent_successfully:
                logger.warning(f"Could not send reminder to {assignee.name} through any channel")
            
        except Exception as e:
            logger.error(f"Error processing reminder for task {task_id}: {e}")
            # エラーが発生してもフラグを設定（重複送信防止）
            DatabaseManager.mark_reminder_sent(task_id)
